<?php

$speakers = array(
  array(
    'id' => 'adil-haris',
    'img' => 'img/speakers/adil-haris.jpg',
    'logo' => 'img/speakers/adil-haris-logo.jpg',
    'name' => 'Adil Haris',
    'title' => 'Product Development Lead and Blockchain Consultant',
    'company' => 'Blockchain Technology Malaysia',
    'description' => [
      'Adil operates as a Product Development Lead and Blockchain Consultant in Blockchain Technology Malaysia, a young startup in Kuala Lumpur, Malaysia specializing in Blockchain consultancy and solutions development. He is a computer science graduate and has been working in the Blockchain space for over three years. His interest in distributed ledger technology stemmed from his early experience working in asset management and investment firms including stock exchanges and mutual fund companies.',
      'He was exposed to and gradually immersed himself in Blockchain during his early years working in Fidelity Investments. He was involved in the analysis and development of a Blockchain pilot project for Fidelity Investment Management Technology (FIMT), dealing with execution of a trade life cycle on a Blockchain. This was followed by projects in Blockchain R&D for FinTech firms where he was able to test the scale and performance of the technology while working with large-scale production data. He has technical experience in multiple Blockchain platforms including Ethereum and Hyperledger along with a proven capability to analyze the feasibility of Blockchain in a wide range of business cases spanning across multiple industries.',
      'A true innovator, through his work and study, he understands the potential of Blockchain, and its promise for disintermediation - removing middlemen and trusted third parties in our society, and instead placing trust on distributed networks secured by cryptography and consensus algorithms. He is passionate about concepts in Blockchain technology which helps garner adoption amongst the public – stablecoins, decentralized autonomous organizations, token curated registries and the building of token economies.'
    ],
  ),
  array(
    'id' => 'nicholas-khaw',
    'img' => 'img/speakers/nicholas-kaw.jpg',
    'logo' => 'img/speakers/nicholas-kaw-logo.jpg',
    'name' => 'Nicholas Khaw',
    'title' => 'Development Economist',
    'company' => 'Khazanah Nasional',
    'description' => [
      'Nicholas Khaw is a development economist at Khazanah Nasional, Malaysia’s sovereign development fund, where he leads the economic development research track. His work at Khazanah covers a wide range of development issues including, but not limited to, industrial policy, cluster development, economic complexity, demographics and political economics. In addition, he also heads the knowledge management team at Khazanah, covering knowledge systems, events, and publications as well as the Khazanah Resource Centre.',
      'Prior to Khazanah, Nick served as an economist in Malaysia’s Economic Planning Unit, where he was directly involved in several national development projects such as the 10th Malaysia Plan, the Economic Transformation Programme, and the Strategic Reform Initiatives. His research interests and intellectual passions are in the field of development economics particularly related to the long-term persistence of historical events and cultural factors on economic and public policy outcomes today.',
      'Nick holds a Master in Public Administration in International Development from the Harvard Kennedy School and completed his undergraduate studies in Economics at Harvard College. He writes a monthly column for The Edge Malaysia. All his writings can be found at www.nicholaskhaw.com',
    ]
  ),
  array(
    'id' => 'tengku-zafrul',
    'img' => 'img/speakers/tengku-zafrul.jpg',
    'logo' => 'img/speakers/tengku-zafrul-logo.jpg',
    'name' => 'YM Tengku Dato’ Sri Zafrul Tengku Abdul Aziz',
    'title' => 'Group Chief Executive Officer/Executive Director',
    'company' => 'CIMB Group Holdings Berhad',
    'description' => [
      'Tengku Dato’ Sri Zafrul Tengku Abdul Aziz is the Group Chief Executive Officer/Executive Director of CIMB Group Holdings Berhad, a leading ASEAN universal bank and a world leader in Islamic finance with presence in 15 countries worldwide. He is also the Chief Executive Officer/Executive Director of CIMB Bank Berhad and Commissioner of PT Bank CIMB Niaga Tbk.',
      'With over 21 years of experience in the financial services sector, specialising in Investment Banking, Zafrul’s last position was with Maybank Investment Bank Berhad and Maybank Kim Eng Holdings as Chief Executive Officer. He also held senior positions in Citigroup Malaysia, Kenanga Holdings Berhad and Avenue Securities. He also experienced being an entrepreneur by setting up Tune Money Sdn Bhd, Asia’s first “no-frills” online financial service provider.',
      'Zafrul graduated from the University of Bristol, United Kingdom with a Bachelor of Science (Hons) Economics and Accounting and obtained a Master of Arts (MA) in Finance and Management from the University of Exeter, United Kingdom. He also holds a Fellow Chartered Banker with the Asian Institute of Chartered Bankers.'
    ]
  ),
);